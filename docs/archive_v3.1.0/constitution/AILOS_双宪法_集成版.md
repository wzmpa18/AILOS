...
文档版本：v2.3.0（双宪法集成版·v1.1.0 迭代体系完善）定稿日期：2026-08-02集成日期：2026-08-02文档状态：✅ Active — v1.1.0 ITERATION COMPLETE维护责任：总工程师 + 监理端（CodeBuddy）联合维护适用范围：所有人类开发者、AI 编程工具、第三方开发者、运维人员、产品人员
...
**第一优先级通道**：腾讯云轻量云「执行命令」控制台。
**永久废弃**：本地 sshpass + PowerShell/CMD 内联命令方案。
**备用通道**：`scp 上传脚本 → ssh 执行` 两步模式（仅限控制台不可用时）。

### 7.2 标准化部署流程

所有生产部署统一执行 `bash /www/xuewaiyu-backend/deploy.sh`，内置六步闭环：

1. **数据库备份**：`pg_dump` 自动备份到 `/www/backups/`
2. **拉取代码**：`git pull origin main`
3. **数据结构同步**：`prisma db push --accept-data-loss` + `prisma generate`
4. **静态资源同步**：`cp -rf public/xuewaiyu/* /www/xuewaiyu/`
5. **服务平滑重启**：`pm2 restart xuewaiyu-backend --update-env`
6. **冒烟验证**：自动校验核心接口 + 核心页面 HTTP 状态码

### 7.3 冒烟验证门禁

部署脚本自动执行以下校验，任一不达标则终止并告警：
- 健康检查：`/api/health` 必须 HTTP 200
- 核心页面：profile/learn/translate/practice/game/membership/about/home 全部 200
- 核心接口：user/me, language/custom/quota, reports/summary, checkin, dashboard, membership/status, ai/quota, practice/config 全部 200

### 7.4 一键回滚

```bash
cd /www/xuewaiyu-backend && git reset --hard <上一个commit> && bash deploy.sh
```
...
- 禁止将生产环境作为测试环境
- 部署前必须有自测证据包
- 连续 2 次出现「上线才发现跑不通」的低级问题，启动交接评估
...
- 词汇、语法、听力、阅读、公共句型练习等所有标准化学习内容，优先查 `LearningContent` 表
- 按「语言 + 等级 + 内容类型」做唯一索引匹配
- 缓存命中直接返回，不调用 AI
- 未命中才调用 AI 生成，生成后自动入库永久缓存
- 通用内容 7 天自动更新一次

### 8.2 自定义语言计费规则

- 系统预设 7 种语言（中英日韩法德西）所有内容走公共缓存，不产生额外 AI 费用
- 用户自定义小语种：基于通用语言模板做 AI 轻量改写，不全新生成
- 前端显示永久免费，AI 生成类服务差异化计费
- 计费通过 `languageBillingService` 扣减 `customLanguageQuota`

### 8.3 AI 调用超时保护

所有 AI 调用必须设置超时保护，超时自动走兜底逻辑：
- 数据库查询超时：3 秒
- 薄弱点查询超时：3 秒
- AI 生成超时：10 秒
- 超时后返回本地兜底内容，接口永远返回 HTTP 200，绝对禁止 500/000

### 8.4 内容质量校验标准

- AI 生成内容入库前必须经过格式校验（JSON 结构完整性）
- 合规校验（无敏感内容）
- 校验失败自动降级到本地兜底库
- 所有 AI 调用记录日志，便于成本核算
...
- 所有代码提交前，必须本地启动服务自测
- 核心接口跑通、目标页面无白屏、无控制台报错
- 产出自测证据包：截图 + Network 零 404 + Console 零报错 + 接口返回验证
- 无证据的提交一律视为无效交付，直接打回

### 9.2 全链路走查验收标准

模拟真实用户完成全链路走查：
1. 登录流程：正常登录、自动登录、退出登录
2. 全页面遍历：底部 7 个 tab + 所有二级页面，逐个打开，无空白、无 404
3. 全链接点验：所有可点击链接，逐个点击，无跳错
4. 核心功能：发音、录音、答题、游戏，逐个使用，功能可用
5. 登录态稳定性：切页、刷新、杀进程重开，均不丢失

### 9.3 敷衍交付问责规则

- 禁止「代码改了就算完成」的敷衍交付
- 所有功能必须有可核验的实际效果，没效果等于没做
- 部署后若出现「页面空白、接口 500、资源 404」等可通过走查避免的低级问题，直接计入严重交付事故
- 累计 2 次同类低级问题，立即启动第三方团队交接评估，所有项目资产全部移交

### 9.4 证据包规范

必交证据清单：
- 网页端：6 核心页面截图 + Network 零 404 + Console 零报错 + 3 接口验证
- 安卓端：返回键逻辑录屏 + 登录持久化录屏 + 滚动流畅录屏
- 支付素材：首页/尾页/功能页/支付页 4 张截图
- 规范文档：宪法增补 + 账簿更新

---

## 增补条款 Z1：第 0 编四大刚性铁律（2026-08-05 P0 图谱审计增补）

> 增补依据：P0 系统图谱一致性校验发现 7 项一级违宪旁路，现行宪法缺少对历史遗留代码、紧急修复例外、未登记功能、整改追踪的刚性约束。增补以下 4 条铁律，具有与正文同等最高强制约束力。

### Z1.1 历史违宪无豁免原则

所有现存直连模型、直写缓存、跨模块耦合代码，一律按一级/二级违宪判定，**无"历史遗留特殊对待"说法**，必须按计划整改闭环。

适用场景：
- `aiService._callDirect()` 直连混元 API
- `hunyuanVisionAdapter` 独立密钥直连 OCR
- `voiceService` 直连腾讯云 TTS/ASR
- `deviceRiskService` / `aiQuotaService` / `authService` 直写 Redis

违宪等级：🔴 一级违宪（不整改不得上市）

### Z1.2 无临时直连通道原则

任何场景（包括线上紧急修复）都不允许业务层直接调用模型 API、直接写 Redis 缓存。紧急修复必须走 Brain 内核降级链路。
...
所有接口、服务、数据表、缓存 Key 必须在 System Atlas 图谱中有登记。未登记的一律判定为非法功能，禁止上线。
...
所有违宪点台账永久留存，整改完成后保留记录，不得删除。作为后续架构变更的参照基准。

追踪要求：
- 每条违宪点绑定：文件路径 + 代码行号 + 宪法条款 + 整改阶段 + 责任人
- 整改完成标记 FROZEN 但记录不删除
- 上市审计时全部违宪记录作为准入依据
...
### Z1.5 审计机制固化规则（2026-08-05 P0 终审增补）

1. **审计执行主体**：安全合规审计中心为唯一审计执行主体，拥有独立审计权，无需提前通知开发端
2. **审计周期**：每月 1 次全量代码扫描（每月第一个工作日）
3. **审计产出**：《月度违宪审计报告》，直接报治理中枢，内容包括：违宪台账核验结果、新增违规项、超期未整改项、问责建议
4. **问责机制**：同类问题重复出现 3 次，自动启动问责流程，对应中心负责人必须复核
5. **审计工具**：基于 system-atlas-v3.0.0.json 机器可读元数据，自动比对代码实际路由/服务与图谱登记项

### Z1.6 Stage 里程碑强制 Deadline 规则

1. 所有 Stage 必须设定刚性交付日期，写入 P1 总账账簿
2. 超期未交付自动触发一级告警，对应中心负责人 48 小时内复核说明原因
3. 连续 2 次超期升级为一级违宪，启动问责
4. 违宪整改项 Deadline 与 Stage 里程碑强绑定，不得晚于对应 Stage 交付日期

### Z1.7 图谱机器可读元数据同步规则

1. 图谱 .md 文件必须配套 `system-atlas-vX.Y.Z.json` 机器可读元数据文件
2. JSON 文件包含：所有模块、路由、服务、数据表、依赖关系、违宪点
3. 图谱 .md 更新时，JSON 必须同步更新（PR 检查），版本号保持一致
4. JSON 作为 CI 违宪扫描脚本的唯一比对基准

### Z1.8 软约束禁止原则

1. 所有治理规则必须配套代码/流程硬拦截，禁止"靠自觉、靠人工"的软约束
2. 软约束 = 没约束 = 给钻空子留后门，一律按治理缺陷处理
3. CI/CD 流水线必须包含：版本号校验、依赖黑白名单校验、直连模型扫描
4. 本地开发环境密钥与生产环境完全隔离，本地仅能调用 Mock 接口

---

> **宪法增补完成。v2.6.0，2026-08-05。总工程师终审签发。P0 图谱 v3.0.0 正式冻结。**
